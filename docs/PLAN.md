...
Tasks:
- [ ] `Deployment` + `Service` manifests in `deploy/k8s/`, raw or Kustomize.
...
- [ ] Gateway runs on `rest.InClusterConfig()` with a projected ServiceAccount
  token, as non-root, with the RBAC already in `gateway-rbac.yaml`. Assert it
  starts on ambient pod credentials and survives token rotation with its watch
  streams intact — the code path exists today and has never run.
- [ ] TLS material delivered as a `Secret`, generated by the existing OpenSSL
...
- [ ] Reachability from outside: a `NodePort` fronted by kind's
...
- [ ] Build and `kind load` the image **once** in `e2e/run_all.sh`, not per gate.
  Per-gate loading would add 20-40s six times over.
- [ ] Keep the inner dev loop. Requiring rebuild, load and rollout for every Go
...
- [ ] **Replace the log-scraping assertions.** `e2e/watch_test.sh` asserts an
...
- [ ] Per-gate isolation: a gateway Pod surviving between gates carries its
...
Tasks:
- [ ] `make docs-generate` emits the reference material above into
  `docs/generated/`.
- [ ] A documentation site published to GitHub Pages, human-written guides
  alongside the generated reference.
- [ ] **An anti-drift CI check, not a diff-police one.** CI runs
...
- [ ] For the human half, a lightweight changeset: a note under `.changes/` for
